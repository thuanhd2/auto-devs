...
func (m *MockProjectRepository) GetWithTasks(ctx context.Context, id uuid.UUID) (*entity.Project, error) {
	args := m.Called(ctx, id)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockProjectRepository) GetProjectStatistics(ctx context.Context, id uuid.UUID) (*entity.ProjectStatistics, error) {
	args := m.Called(ctx, id)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*entity.ProjectStatistics), args.Error(1)
}
...
func (m *MockProjectRepository) GetSettings(ctx context.Context, id uuid.UUID) (*entity.ProjectSettings, error) {
	args := m.Called(ctx, id)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func TestTaskUsecase_UpdateStatusWithHistory(t *testing.T) {
	...
	// Set up expectations
	mockTaskRepo.On("GetByID", ctx, taskID).Return(currentTask, nil).Twice()
	mockTaskRepo.On("UpdateStatusWithHistory", ctx, taskID, entity.TaskStatusPLANNING, &changedBy, &reason).Return(nil)
	mockTaskRepo.On("GetByID", ctx, taskID).Return(updatedTask, nil)
