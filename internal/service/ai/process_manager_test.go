...
func TestProcess_GetOutput(t *testing.T) {
	...
	process, err := pm.SpawnProcess("echo 'stdout message' && echo 'stderr message' >&2", tempDir, "", nil)
	if err != nil {
		t.Fatalf("Failed to spawn process: %v", err)
	}
...
func TestProcessManager_EnvironmentVariables(t *testing.T) {
	...
	process, err := pm.SpawnProcess("echo $AI_PROCESS_ID && echo $AI_WORK_DIR", tempDir, "", nil)
	if err != nil {
		t.Fatalf("Failed to spawn process: %v", err)
	}
...
func TestProcessManager_WorkingDirectory(t *testing.T) {
	...
	process, err := pm.SpawnProcess("cat test.txt", tempDir, "", nil)
	if err != nil {
		t.Fatalf("Failed to spawn process: %v", err)
	}
