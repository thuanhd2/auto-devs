...
const parseMarkdown = (content: string): string => {
  ...
  let html = content
    ...
    .replace(/^## (.*$)/gm, '<h2 class="text-xl font-semibold mt-8 mb-4 text-gray-900 border-b border-gray-200 pb-2">$1</h2>')
    ...
    // Code blocks
    .replace(/```(\w+)?\n([\s\S]*?)\n```/g, '<pre class="bg-gray-100 border rounded-lg p-4 text-sm font-mono overflow-x-auto my-4"><code class="language-$1">$2</code></pre>')
    ...
    // Blockquotes
    .replace(/^> (.*$)/gm, '<blockquote class="border-l-4 border-blue-500 pl-4 my-4 text-gray-700 italic">$1</blockquote>')
  ...
  // Fix list formatting
  html = html.replace(/<\/p><p class="mb-4">(<li class="ml-4">.*?)<\/p>/g, '<ul class="list-disc list-inside space-y-1 my-4">$1</ul>')
  html = html.replace(/(<li class="ml-4">.*?)<br>(<li class="ml-4">)/g, '$1</li>$2')
...
export function PlanPreview({ 
  ...
  if (!content.trim()) {
    return (
      <Card className={`p-8 text-center ${className}`}>
        ...
        <p className="text-sm text-gray-500">Start writing your plan to see a preview here.</p>
      </Card>
    )
  }
  ...
  return (
    <div className={`${className}`}>
      ...
      {/* Content */}
      <div className={`prose prose-sm max-w-none ${printFriendly ? 'print:prose-print' : ''}`}>
        <div 
      ...
      {!printFriendly && (
        <>
          ...
          <div className="flex items-center justify-between text-xs text-gray-500">
            <span>
              Rendered from Markdown • {content.length} characters
            </span>
            <span>
