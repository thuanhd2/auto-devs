...
export function GitOperationControls({
  ...
  const canOpenWorktree = task.git_info?.worktree_path && ['WORKTREE_CREATED', 'BRANCH_CREATED', 'CHANGES_PENDING', 'CHANGES_STAGED', 'CHANGES_COMMITTED'].includes(gitStatus)
  const canCleanupWorktree = task.git_info?.worktree_path && !['NO_GIT', 'WORKTREE_PENDING'].includes(gitStatus)
              ...
              <div className="space-y-2 text-xs text-gray-600">
                {task.git_info.worktree_path && (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-mono truncate" title={task.git_info.worktree_path}>
                      {task.git_info.worktree_path}
                    </span>
                  </div>
                )}
                ...
                {task.git_info.last_sync && (
                  <div className="text-gray-500">
                    Last sync: {new Date(task.git_info.last_sync).toLocaleString()}
                  </div>
                )}

                {(task.git_info.commits_ahead || task.git_info.commits_behind) && (
...
export function getOperationRecommendations(gitStatus: TaskGitStatus): string[] {
