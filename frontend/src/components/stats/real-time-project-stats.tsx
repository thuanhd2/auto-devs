...
export function RealTimeProjectStats({ 
  ...
  const [previousStats, setPreviousStats] = useState<TaskStatistics | null>(null)
  ...
  const stats = useMemo(() => {
    ...
    tasks.forEach(task => {
      ...
      if ((task as any).due_date && new Date((task as any).due_date) < new Date() && task.status !== 'DONE') {
        overdue++
      }
    })
  ...
  useEffect(() => {
    if (previousStats) {
      ...
      if (stats.completionRate !== previousStats.completionRate) changedStats.add('completion')
      if (stats.todayCreated !== previousStats.todayCreated) changedStats.add('todayCreated')
      if (stats.todayCompleted !== previousStats.todayCompleted) changedStats.add('todayCompleted')
      if (stats.overdue !== previousStats.overdue) changedStats.add('overdue')
      if (stats.activeUsers !== previousStats.activeUsers) changedStats.add('activeUsers')

      Object.keys(stats.byStatus).forEach(status => {
        if (stats.byStatus[status as TaskStatus] !== previousStats.byStatus[status as TaskStatus]) {
      ...
      changedStats.forEach(statKey => {
        const element = document.querySelector(`[data-stat="${statKey}"]`) as HTMLElement
        if (element) {
        ...
        <CardContent>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span 
                className={`font-medium transition-all duration-300 ${
                  animatingStats.has('todayCreated') ? 'scale-110 text-blue-600' : ''
                }`}
            ...
            <div className="flex items-center justify-between">
              ...
              <span 
                className={`font-medium transition-all duration-300 ${
                  animatingStats.has('todayCompleted') ? 'scale-110 text-green-600' : ''
                }`}
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <span 
                className={`font-medium transition-all duration-300 ${
                  animatingStats.has('activeUsers') ? 'scale-110 text-green-600' : ''
                }`}
...
export function CompactProjectStats({ projectId, tasks }: Pick<ProjectStatsProps, 'projectId' | 'tasks'>) {
  const stats = useMemo(() => {
    ...
    return {
      ...
      completed,
      completionRate,
      inProgress: tasks.filter(t => ['IMPLEMENTING', 'CODE_REVIEWING'].includes(t.status)).length,
    }
  }, [tasks])
