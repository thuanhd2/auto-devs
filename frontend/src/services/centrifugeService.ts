import { Centrifuge, PublicationContext, SubscriptionDataContext } from 'centrifuge'
...
export class CentrifugeService {
  ...
  connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const wsUrl = `${protocol}//${this.config.url}/ws/connect`
  ...
  isConnected(): boolean {
    return this.connectionState.status === 'connected' && this.centrifuge !== null
  }
  ...
  send(message: any): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      if (message.type === 'subscription') {
        ...
        const method = action === 'subscribe' ? 'subscribe_project' : 'unsubscribe_project'
        
        this.centrifuge.rpc(method, { project_id })
          .then(() => resolve())
          .catch(reject)
      } else {
